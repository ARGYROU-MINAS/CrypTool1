	...
	if (EingabeDialog.DoModal() == IDOK)
	{
		...
		else
		{
			...
			else if (hill_rc == HILL_UNDEF)
			{
				...
				for (int i=EingabeDialog.von; i<=EingabeDialog.bis; i++)
				{
					zahl.Format("%d", i);
...
// == display the ascertained XOR-key
	...
	for(int i=1;i<periode;i++) { // check for repeated keys
		if(periode%i == 0) {
			if(!memcmp(key,key+i,periode-i)) { // iterated key found
				periode = i;
				for(i=0;i<periode;i++) sprintf(line2+(i*3)," %02.2X", key[i]);
...
// == Decrypt the ciphertext 	
	for(int i=0;i<periode;i++) sprintf(line2+(i*3)," %02.2X", key[i]);
	SymbolArray KeyArray(IdConv,periode);
	for(int i=0;i<periode;i++) KeyArray[i]=key[i];
	// Entschlüsseln
	text -= KeyArray;
